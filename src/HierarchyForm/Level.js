...
export const Level = React.forwardRef( ( { options, handleCollapsible, isRootLevel = false }, ref ) => {
	...
	const onChangeHandler = React.useCallback( ( actionType, value ) => {
		...
		if ( oid )
			handleOnChange( actionType, { oid, isAdd, label, levelKey, parentKey, rootOid, childKey, parentOid, parentIndex, currentIndex, parentUniqueKey, uniqueKey, isRootLevel } )
	...
	return (
		<Form.Group className="level">
			<div
				ref={ collapsibleRef }
				onClick={ () => isRootLevel && onClickCollapsibleHandler() }
				className={ `icon-container${ ( !isRootLevel || isAdd ) ? ' icon-hidden' : '' }${ isRootLevel ? ` ${ rootLevelKey }__collapsible` : '' }` }
			>
